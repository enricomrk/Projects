...
def build_argparser():
    ...
    args.add_argument('-w', '--hide', help='Do not show output image', required=False, action='store_false')
    return parser
...
def main():
    ...
    if has_video:
        ...
        if args.save:
            writer = cv2.VideoWriter(outdir + basename(args.video) ,cv2.VideoWriter_fourcc(*'mp4v'), fps, size)
    else:
    ...
    while True:
        ...
        for number, proposal in enumerate(res):
            if proposal[2] > 0:
                ...
                if confidence > args.confidence:
                    output.append((xmin, ymin, xmax, ymax, label, basename(name), confidence))
                    """if not args.save:
        ...
        if args.save:
            if has_video:
                ...
                cv2.imwrite(outdir + 'detection_' + model_name + '_' + base, image)
        else:
            if args.hide:
